...
- Root `--hpx` mode is implemented for supported data commands. It emits canonical HyperContext NDJSON for profile, sleep, recovery, cycles, and workouts, using deterministic `origin_id` values and stable `source = "whoop"`.
...
- Next up: help-text/examples polish for the new HPX mode, any HyperContext schema expansion for currently unregistered WHOOP fields, and any remaining distribution niceties (e.g., Windows builds).
